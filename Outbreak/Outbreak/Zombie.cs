...
namespace OutbreakManager
{
    public class Zombie : Moveable
    {
        ...
        public void CheckSurroundings(List<Human> inProximity)
        {
            foreach (Human human in inProximity)
            {
                Vector3 spotVector = Vector3.Normalize(human.location - this.location);
                double spotAngle = Math.Acos(Vector3.Dot(spotVector, this.lookDirection));

                if (spotAngle >= -MAX_VIEW_ANGLE && spotAngle <= MAX_VIEW_ANGLE)
                {
                    if (targetVictim == null || (human.location - this.location).Length() < (targetLocation - this.location).Length())
                    {
                        targetVictim = human;
                        targetLocation = human.location;
                    }
                }
            }
        }
